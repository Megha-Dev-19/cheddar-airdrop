...
async function ftDropNear(){
	// Initiate connection to the NEAR mainnet blockchain.
	const network = "mainnet"
	const CREDENTIALS_DIR = ".near-credentials";
	const credentialsPath =  path.join(homedir, CREDENTIALS_DIR);
	const YOUR_ACCOUNT = "meghagoel.near";
	const FT_CONTRACT = "token.cheddar.near";
	...
	console.log(funderFungibleTokenBal, amountToTransfer)
	...
	try {
		const txn = await fundingAccount.functionCall({
			contractId: KEYPOM_CONTRACT, 
			methodName: 'create_drop', 
			args: {
				public_keys: pubKeys,
				deposit_per_use: parseNearAmount("0"),
				ft: {
					...
					balance_per_use: amountToTransfer
				}
			}, 
			gas: "300000000000000",
	...
	try {
		// const txn2 = await fundingAccount.functionCall({
		// 	contractId: FT_CONTRACT, 
	...
	var dropInfo = {};
	    ...
	    let linkdropUrl = `https://mynearwallet.com/linkdrop/${KEYPOM_CONTRACT}/${keyPair.secretKey}`;
